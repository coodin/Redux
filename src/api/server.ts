...
import { FactoryAPI, UpdateManyValue } from "@mswjs/data/lib/glossary"
import { keysForReactionButtonsType } from "../features/posts/ReactionButtons"
...
export const handlers = [
  ...
  rest.get("/fakeApi/posts/:postId", function (req, res, ctx) {
    const post = db.post.findFirst({
      where: { id: { equals: req.params.postId.toString() } },
    })
    return res(ctx.delay(ARTIFICIAL_DELAY_MS), ctx.json(serializePost(post)))
  }),
  rest.patch("/fakeApi/posts/:postId", async (req, res, ctx) => {
    const { id, ...data } = await req.json()
    //const updateData: Partial<UpdateManyValue<typeof data>>
  ...
  rest.post("/fakeApi/posts/:postId/reactions", async (req, res, ctx) => {
    const postId =
      typeof req.params.postId === "string" ? req.params.postId : ""
    const reaction = (
      await req.json<{ reaction: keysForReactionButtonsType }>()
    ).reaction
    const post = db.post.findFirst({
      where: { id: { equals: postId } },
    })

    if (post === null || post.reactions === undefined)
      throw Error(
        "There is no post related with this ID or you have no reactions",
      )
...
const sendMessage = (
  ...
  obj: {
    type: string
    payload: { id: string; date: any; message: any; user: any }[]
  },
) => {
...
const sendRandomNotifications = (socket: Client, since: any) => {
  const numNotifications = getRandomInt(1, 5)
